...
INSERT INTO `Member`
VALUES ('b101', '을지문던', '010-5555-1234', '사장', 107, NOW());
...
SELECT `uid`, COUNT(*) AS `건수` FROM `Sales`
GROUP BY `uid`;
...
SELECT `uid`, AVG(`sale`) AS `평균`FROM `Sales`
	GROUP BY `uid`;
...
# 확인문제2
SELECT `name`, SUM(`sale`) AS `2019년도 매출합`
FROM `Sales` AS a
JOIN `Member` AS b
ON a.uid = b.uid
